...
const MisRecetas = () => {
  ...
  const obtenerRecetas = async () => {
    ...
    try {
      const response = await axios.get(
        ` http://localhost:5000/api/recetas/buscar?ingrediente=${ingrediente}&nivel=${nivel}&maxTiempo=${maxTiempo}&usuarioId=${userId}`);
  ...
  const eliminarReceta = async (eliminarId) => {
    ...
    if (confirmacion) {
      try {
        await axios.delete(
          `http://localhost:5000/api/recetas/eliminar/${eliminarId}`,
          {
            headers: {
